                    ...
                    if (Player* player = m_caster->ToPlayer())
                    {
                        ...
                        if (uint32 combo = player->GetComboPoints())
                        {
                            ...
                            if (AuraEffect const* aurEff = unitTarget->GetAuraEffect(SPELL_AURA_PERIODIC_DAMAGE, SPELLFAMILY_ROGUE, 0x00010000, 0, 0, m_caster->GetGUID()))
                            {
                                ...
                                damage *= doses;
                                damage += int32(player->GetTotalAttackPowerValue(BASE_ATTACK) * 0.11f * combo);damage += int32(player->GetTotalAttackPowerValue(BASE_ATTACK) * 0.11f * combo);damage += int32(player->GetTotalAttackPowerValue(BASE_ATTACK) * 0.11f * combo);
                            }
